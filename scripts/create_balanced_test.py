...
def create_balanced_test_set(
    ...
    if len(code_samples) < target_code:
        print(f"\n⚠️  Code样本不足: 需要{target_code}, 只有{len(code_samples)}")
        print(f"   将重复使用Code样本")
        # 重复Code样本
        repetitions = target_code // len(code_samples)
        remainder = target_code % len(code_samples)
        code_samples = code_samples * repetitions + random.sample(code_samples, remainder)
    ...
    selected_code = code_samples[:target_code]  # 使用重复后的样本
...
if __name__ == "__main__":
    create_balanced_test_set(
        ...
        output_file="data/test/balanced_test_dataset.jsonl",
        target_total=2000,  # 2k测试集
        math_ratio=0.40,
        code_ratio=0.30,
        qa_ratio=0.30
    )
